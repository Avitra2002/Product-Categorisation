...
def process_social_media_data(file_path, product, source):
    ...
    try:
        ...
        if file_path_str.lower().endswith('.xls') or file_path_str.lower().endswith('.xlsx'):
    ...
    for col in data.columns:
        ...
        #     print(f"Date column before formatting: {data[date_column]}")
        #     data['formatted_date'] = data[date_column].apply(lambda x: format_date(x))
        #     print(f"Date column after formatting: {data['formatted_date']}")
        #     # data['formatted_date'] = data[col].apply(format_date)
        #     # print("Date column after formatting:", data['formatted_date'])
       
        if 'date' in col.lower() and data[col].apply(lambda x: isinstance(x, str) and '/' in x).all():
            date_column = col
            print(f"Date column identified: {col}")
            print(f"Date column before formatting: {data[col]}")
            data['formatted_date'] = data[date_column].apply(lambda x: format_date(x))
            print(f"Date column after formatting: {data['formatted_date']}")
            break
            #data[col] = data[col].apply(format_date)
           # print(f' data[col] is {data[col]}')
    ...
    for col in data.columns:
        if 'date' in col.lower() and data[col].apply(lambda x: isinstance(x, str) and '/' in x).all():
            date_column = col
            print(f"Date column before formatting: {data[date_column]}")
            data[date_column] = data[date_column].apply(lambda x: format_date(x))  # Overwrite the original date column
            print(f"Date column after formatting: {data[date_column]}")
            break
        elif 'date' in col.lower() and data[col].apply(lambda x: isinstance(x, str) and '-' in x).all():
            date_column = col
            print(f"Date column before formatting: {data[date_column]}")
            ...
            print(f"Date column after formatting: {data[date_column]}")
            break
    print(f"All columns in the DataFrame: {data.columns}")
    
    for col in data.columns:
        if 'feedback' in col.lower():
            feedback_column = col
            print(f"Feedback column identified: {col}")
            break
    # if feedback_column:
    ...
    if feedback_column:
        print("Evaluating each row:")
        for index, row in data.iterrows():
            is_not_na = pd.notna(row[feedback_column])
            is_eng = is_english(row[feedback_column])
            is_valid = is_valid_feedback(row[feedback_column])
            print(f"Row {index}: notna={is_not_na}, is_english={is_eng}, is_valid_feedback={is_valid}")

        print(f"DataFrame before filtering:\n{data}")
        filtered_data = data[data[feedback_column].apply(lambda x: pd.notna(x) and is_english(x) and is_valid_feedback(x))]
